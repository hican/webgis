...
//********************加载离线arcgis瓦片地图*************************//
var baseMapPath = "http://localhost:8080/map/";  //基础电子地图路径
...
source_sqlite_offline = new ol.source.XYZ({
    tileUrlFunction: function (tileCoord) {
        if (tileCoord) {
            ...
            return "http://" + window.location.host + window.location.pathname + "MBTilesServlet?T=GMap&L=" + z + "&X=" + x + "&Y=" + y;
        } else {
            return '';
        }
    }
...
source_sqlitesat_offline = new ol.source.XYZ({
    tileUrlFunction: function (tileCoord) {
        if (tileCoord) {
            ...
            var y = -tileCoord[2] - 1;
            y = 2 * (Math.pow(2, (z - 1)) - y) - 1 + y;
            return "http://" + window.location.host + window.location.pathname + "MBTilesServlet?T=GMapSat&L=" + z + "&X=" + x + "&Y=" + y;
        } else {
            return '';
        }
    }
